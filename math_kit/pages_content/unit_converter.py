...
class UnitConverterPage:
    ...
    def __init__(self, master, tools):
        ...
        self.unit_converter_frame = Frame(
        ...
        self.quantity_box = ttk.Combobox(
            self.container,
        ...
        self.clear_img_button = Button(
            self.container,
            image=self.clear_icon_img,
            bg=tools.pallete["blue"],
            bd=1,
            cursor="hand2",
            activebackground=tools.pallete["purple"],
        ...
        self.unit_from_box = ttk.Combobox(
            self.container,
            textvariable=self.from_variable,
            state="readonly",
            font=("Helvetica", 15),
        )

        self.to_label = Label(
            self.container, text="to", font=("Helvetica", 25), bg=tools.pallete["gray"]
        )

        self.to_variable = StringVar()
        self.unit_to_box = ttk.Combobox(
            self.container,
            textvariable=self.to_variable,
            state="readonly",
            font=("Helvetica", 15),
        )
        ...
        self.flip_img_button = Button(
            self.container,
            image=self.flip_icon_img,
            bg=tools.pallete["blue"],
            bd=1,
            cursor="hand2",
            activebackground=tools.pallete["purple"],
        ...
        self.result_text = Entry(
            self.container,
            textvariable=self.result_variable,
            font=("Helvetica", 15),
            width=int(tools.screen_width * 0.04),
            bg=tools.pallete["dark blue"],
            fg=tools.pallete["white"],
        )

        clear_result_image_size = int(tools.screen_width * 0.02)
        self.clear_result_icon = Image.open(r"../math_kit/assets/icons/clear.png")
        self.clear_result_icon = self.clear_result_icon.resize(
            (clear_result_image_size, clear_result_image_size)
        )
        self.clear_result_icon_img = ImageTk.PhotoImage(self.clear_result_icon)
        self.clear_result_img_button = Button(
            self.container,
            image=self.clear_result_icon_img,
            bg=tools.pallete["blue"],
            bd=1,
            cursor="hand2",
            activebackground=tools.pallete["purple"],
    ...
    def _Flip(self):
        ...
        try:
            ...
            messagebox.showerror(
                "Invalid Selections !",
                "You have to fill both fields with valid selections.",
            )
    ...
    def _Convert(self, *args):
        ...
        try:
            ...
            messagebox.showerror(
                "Invalid Selections !",
                "You have to fill all fields with valid selections.",
            )
